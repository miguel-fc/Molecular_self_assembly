...
;Troisi et al., PNAS, 102, 255-260 (2005)
; This code is based on one taken from the Netlogo repository but improved
; in several ways so as to reproduce the results in the paper reference above 
...
globals [
  ...
  itrans-accepted  ;; total number of translational movements accepted
]
breed [nodes node]
breed [walkers walker]
walkers-own [location leader goodnodes probability walker-energy]  
...
ask walkers with [leader = self] [link-all]
...
;Initializing values 
        set goodnodes []
        set probability 0
        set goodnodes lput location goodnodes   
        find_global-energy
...
;stay put but do a rotation
        if movement = 0 [
          ...
          ifelse global-energy <= previous-global-energy [ ][
            ...
            ifelse random-float 1 < probability [ ] [ rt -1 * rot1 * 90]
          ]
        ]            
        ...
        if movement = 1 [
          ...
          [ compute-probability-of-this-movement         
            ifelse random-float 1 < probability [  
              ...
              ifelse global-energy <= previous-global-energy [ ][
              compute-probability-of-this-movement
              ifelse random-float 1 < probability [ ] [ rt -1 * rot2 * 90]
              ]
            ] 
            [ set-location one-of [link-neighbors at-points [[0 1] ] ] of location]
          ]
...
;If none of the moves was energetically accepted, 
;just make one where collision does not occur. 
;Do a rotation in multiples of 90 degrees as well 
 
;The get out of the hole move and the rotation          
;        let ifindrot 0
;        if left_right = 0 and up_down = 0 [ move-to-a-no-collision-place-and-rotate ]
...
to find_global-energy
  ...
  ask walkers [
    ...
    if [color] of self  = blue [                     ;show "blue"
      if any? cands_on_neighbs with [color = blue] [set global-energy global-energy + (9 / 2)]
      if any? cands_on_neighbs with [color = red] [set global-energy global-energy - (11 / 2)]
      if any? cands_on_neighbs with [color = black] [set global-energy global-energy - (1 / 2)]
      ]
    
    if [color] of self  = red [ 
      if any? cands_on_neighbs with [color = blue] [set global-energy global-energy - (11 / 2)]
      if any? cands_on_neighbs with [color = red] [set global-energy global-energy + (9 / 2)]
      if any? cands_on_neighbs with [color = black] [set global-energy global-energy - (1 / 2)]
      ]
    
    if [color] of self  = black [ 
      if any? cands_on_neighbs with [color = blue] [set global-energy global-energy - (1 / 2)]
      if any? cands_on_neighbs with [color = red] [set global-energy global-energy - (1 / 2)]
      if any? cands_on_neighbs with [color = black] [set global-energy global-energy - (1 / 2)]
      ]
  ]
...
; Evaluates the energy of a translational movement and whether or not a collision even took place
; If a collision does not take place, the corresponding node is stored in the list goodnodes
...
to move-to-a-no-collision-place-and-rotate
  let ifindrot 0
  let value random length goodnodes
  set-location item value goodnodes
  let rot1 random 4
  rt rot1 * 90  
  if any? link-neighbors with [leader = [leader] of myself] [
    ask link-neighbors with [leader = [leader] of myself] [
